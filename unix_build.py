...
version = Version(*map(int, cfg.get("metadata", "version").split(".")))
...
def cc_version():
    ...
    m = re.search(rb"^#define __clang__ 1", raw, flags=re.M)
    cc_name = "gcc" if m is None else "clang"
    ver = (
        int(re.search(rb"#define __GNUC__ (\d+)", raw, flags=re.M).group(1)),
        int(re.search(rb"#define __GNUC_MINOR__ (\d+)", raw, flags=re.M).group(1)),
    )
...
def init_env(debug=False, sanitize=False, native_optimizations=False, add_python=True):
    ...
    ldflags = os.environ.get(
        "OVERRIDE_LDFLAGS",
        "-Wall -shared " + " ".join(sanitize_args) + ("" if debug else " -O3"),
    )
...
def build(args, build_leak_check=False):
    ...
    if newer(TEST_EXE, *debug_objects):
        cmd = (
            [debug_env.cc] + debug_env.cflags + ["test.c"] + ["-o", TEST_EXE] + ldflags
        )
        run_tool(cmd)
...
def main():
    ...
    if args.action == "build":
        ...
        os.execlp(
            ...
            TEST_EXE,
            "-c",
            "from html5_parser import *; " + args.rest[0],
            *args.rest[1:],
        )
